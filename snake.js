...
class SnakeGame {
  ...
  drawHUD() {
    ...
    this.context.fillText("Score: "+this.score+" / "+this.maxScore, this.screenWidth/2, this.screenHeight-this.hudHeight/2+2);

    if ( this.score == this.maxScore )  {
      this.context.font = "16pt Courier New";
      this.context.fillStyle = "white";
      this.context.textAlign = "center";
      this.context.fillText("You Win! Congratulatons!", this.screenWidth/2, this.screenHeight/2);
    }
  }
  ...
  update() {
    if (this.score == this.maxScore) { // Won the game
      return;
    }
    ...
    if ( head.x<0 || head.y<0 || head.x>=this.mapWidth || head.y>=this.mapHeight ) { // hit wall
      game.reset();
    }
  ...
  gameTick() {
    ...
    this.draw();
    let game = this;
    setTimeout(function() { game.gameTick(); }, this.timerDelay);
  }
